...
void displayPaws_P22D(Mat img_bottom, Mat img_top, vector<P22D> pair_candidates);
...
vector<P22D> matchViews(const Mat& boolD, const Mat& D_top_weight, vector<Candidate>& C_b, int N_bottom, vector<Candidate>& C_t, int N_top, bool vel_check, LocoMouse_Feature F, const Mat &Ibbb, const Mat &Itbb, const Mat &Ibbb_prev, const Mat &Itbb_prev, bool debug);
...
MATSPARSE pairwisePotential(vector<Candidate> &Ci, vector<Candidate> &Cip1, Point_<double> &ONG_BR_corner, double grid_spacing, vector< Point_<double> > &ONG, Size ONG_size, double max_displacement, double alpha_vel, double pairwise_occluded_cost);

MATSPARSE pairwisePotential_top(vector<uint> &Zi, vector<uint> &Zip1, double grid_mapping, double grid_spacing, vector<uint> &ONGi, uint Nong, double max_displacement, double alpha_vel, double pairwise_occluded_cost);

uint32_t matchToRange(int32_t input, int32_t min_val, int32_t max_val);

Point medianPoint(int* x, int* y, const int N);

void mouseBoundingBox_postProcess(vector<double> &bb_w, vector<double> &bb_hb, vector<double> &bb_ht, int N_frames, Rect &BB_top, Rect &BB_bottom);
...
Mat detectTail(Mat &I_bb_bottom_pad, Mat &I_bb_top_pad, Rect BB_tail_bottom_pad, Rect BB_tail_top_pad, Rect BB_tail_bottom_unpad, Rect BB_tail_top_unpad, Mat &Filtered_tail_bottom, Mat &Filtered_tail_top, LocoMouse_Feature F, bool debug);

void imBorderPreservingBB(const Mat& I, Mat& Ibb, Rect BB, uint32_t bb_x, uint32_t bb_y);

void sideViewMatching(Mat& T, vector<Mat> &T_side, vector< vector<P22D> > &candidates_bottom_top_matched, vector<uint> &ONG_top, uint lowest_point, bool debug);

void exportTracks(Mat& M_bottom, const vector<Mat> &M_top, vector< vector<P22D> > &candidates_bottom_top_matched, FileStorage output, Rect BB_bottom, Rect BB_top, vector<uint> bb_x_avg, vector<uint> bb_yb_avg, vector<uint> bb_yt_avg, string feature_name);
